...
// Фиды для Яндекса
Route::prefix('feeds/yandex')->name('feeds.yandex.')->group(function () {
    Route::get('/', [App\Http\Controllers\YandexFeedController::class, 'index'])->name('index');
    Route::get('/masterclasses.xml', [App\Http\Controllers\YandexFeedController::class, 'masterclasses'])->name('masterclasses');
    Route::get('/education.xml', [App\Http\Controllers\YandexFeedController::class, 'education'])->name('education');
    Route::get('/services.xml', [App\Http\Controllers\YandexFeedController::class, 'services'])->name('services');
    Route::get('/activities.xml', [App\Http\Controllers\YandexFeedController::class, 'activities'])->name('activities');
    Route::get('/combined.xml', [App\Http\Controllers\YandexFeedController::class, 'combined'])->name('combined');
    
    // Добавляем маршрут для YML-фида
    Route::get('/yml.xml', [App\Http\Controllers\YandexFeedController::class, 'yml'])->name('yml');
    
    // Маршрут для обновления кэша YML-фида
    Route::get('/refresh-yml', [App\Http\Controllers\YandexFeedController::class, 'refreshYml'])
        ->name('refresh-yml')
        ->middleware(['auth', 'admin']);
    
    // Добавляем маршрут для Яндекс Дзен
    Route::get('/zen.xml', [App\Http\Controllers\YandexFeedController::class, 'zen'])->name('zen');
});

// Добавляем новый маршрут для обновления фида Яндекса
Route::get('/feeds/refresh-yandex', [App\Http\Controllers\YandexFeedController::class, 'refresh'])->name('feeds.refresh-yandex');

// Маршруты для Yandex фидов
Route::get('feeds/yandex/recipes.xml', [App\Http\Controllers\YandexFeedController::class, 'index']);
Route::get('feeds/yandex/combined.xml', [App\Http\Controllers\YandexFeedController::class, 'combined']);
...
// Маршрут для Telegram вебхука с поддержкой разных методов
Route::match(['get', 'post'], '/telegram/webhook', [TelegramWebhookController::class, 'handle']);

// Добавляем маршрут для проверки статуса бота (можно использовать для отладки)
