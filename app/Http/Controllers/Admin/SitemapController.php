...
class SitemapController extends Controller
{
    ...
    public function generate(Request $request)
    {
        try {
            ...
            return redirect()->route('admin.sitemap.index')
                ->with('error', 'Ошибка при генерации карт сайта: ' . $e->getMessage());
        }
    }
    ...
    private function generatePaginationSitemap()
    {
        ...
        $recipesCount = Recipe::where('status', 'published')->count();
        ...
        foreach ($categories as $category) {
            $categoryRecipesCount = $category->recipes()->where('status', 'published')->count();
