...
class NewsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // SEO настройки
        $title = 'Кулинарные новости и статьи';
        $description = 'Последние новости из мира кулинарии, интересные факты о продуктах и рецептах, советы шеф-поваров';
        
        if (!empty($searchTerm)) {
            $title = "Поиск новостей: {$searchTerm}";
            $description = "Результаты поиска новостей по запросу '{$searchTerm}'";
        }
        
        $this->seoService->setTitle($title)
            ->setDescription($description)
            ->setKeywords('кулинарные новости, новости кулинарии, кулинарные статьи, кухня, питание, рецепты')
            ->setCanonical(url()->current())
            ->setOgType('website')
            ->setOgImage(asset('images/og-news.jpg'));

        // JSON-LD разметка для страницы с коллекцией новостей
        $jsonLd = [
        ...
        return view('news.index', compact('news', 'searchTerm', 'popularNews', 'recommendedRecipes'));
    }
