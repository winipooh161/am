...
<head>
    ...
    <!-- Meta Tags -->
    <meta name="description" content="{{ app(App\Services\SeoService::class)->getDescription() }}">
    @if(app(App\Services\SeoService::class)->getKeywords())
    <meta name="keywords" content="{{ app(App\Services\SeoService::class)->getKeywords() }}">
    @endif
    
    <!-- Canonical URL -->
    @if(app(App\Services\SeoService::class)->getCanonical())
    <link rel="canonical" href="{{ app(App\Services\SeoService::class)->getCanonical() }}">
    @endif
    ...
    <!-- Open Graph -->
    <meta property="og:title" content="{{ app(App\Services\SeoService::class)->getTitle() ?: config('app.name') }}">
    <meta property="og:description" content="{{ app(App\Services\SeoService::class)->getDescription() }}">
    <meta property="og:type" content="{{ app(App\Services\SeoService::class)->getOgType() }}">
    <meta property="og:url" content="{{ url()->current() }}">
    @if(app(App\Services\SeoService::class)->getOgImage())
    <meta property="og:image" content="{{ app(App\Services\SeoService::class)->getOgImage() }}">
    @else
    ...
    <meta name="twitter:title" content="{{ app(App\Services\SeoService::class)->getTitle() ?: config('app.name') }}">
    <meta name="twitter:description" content="{{ app(App\Services\SeoService::class)->getDescription() }}">
    @if(app(App\Services\SeoService::class)->getOgImage())
    <meta name="twitter:image" content="{{ app(App\Services\SeoService::class)->getOgImage() }}">
    @else
    ...
    @if(app(App\Services\SeoService::class)->getSchema())
    <script type="application/ld+json">
        {!! json_encode(app(App\Services\SeoService::class)->getSchema()) !!}
    </script>
    @endif
  ...
  <!-- Preconnect and DNS-Prefetch для ускорения загрузки -->
    ...
    <!-- SEO-метаданные -->
    @include('seo.search_seo', ['query' => $query ?? '', 'results' => $results ?? null])
    
    <!-- Schema.org структурированные данные -->
    @include('schema_org.search_schema', ['query' => $query ?? '', 'results' => $results ?? null])
