...
$title = $metaTags['title'] ?? ($category->meta_title ?? ($category->name . ' - рецепты на Яедок'));
...
$keywords = $metaTags['meta_keywords'] ?? ($category->meta_keywords ?? ($category->name . ', рецепты, кулинария, готовка'));
...
@includeIf('schema_org.category_schema', ['category' => $category, 'recipes' => $recipes])
