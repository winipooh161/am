...
@section('keywords', $keywords ?? 'поиск, рецепты')
...
@section('meta_tags')
    ...
    <link rel="canonical" href="{{ $canonicalUrl ?? route('search') }}" />
...
<div class="container py-4">
    <div class="row">
        <div class="col-lg-8">
            <h1 class="mb-4">{{ !empty($query) ? 'Поиск: ' . $query : 'Поиск рецептов' }}</h1>
            
            <!-- Форма поиска -->
            <div class="card mb-4 border-0 shadow-sm">
                <div class="card-body">
                    <form action="{{ route('search') }}" method="GET" class="row g-3" id="search-form">
                        <div class="col-md-8">
                            <div class="input-group">
                                <span class="input-group-text"><i class="fas fa-search"></i></span>
                                <input type="text" name="query" class="form-control" value="{{ $query ?? '' }}" placeholder="Введите название блюда или ингредиент..." autofocus>
                        ...
                        <div class="col-12">
                            ...
                            <div class="text-center mt-2">
                                <button class="btn btn-link text-decoration-none" type="button" data-bs-toggle="collapse" data-bs-target="#advancedFilters">
                                    <span class="when-collapsed"><i class="fas fa-sliders-h me-1"></i> Дополнительные фильтры</span>
                                    <span class="when-expanded"><i class="fas fa-chevron-up me-1"></i> Скрыть фильтры</span>
                                </button>
                            </div>
                        </div>
                    </form>
                    ...
                    <div class="d-flex flex-wrap align-items-center">
                        ...
                        @if(!empty($query))
                            <span class="badge bg-primary me-2 mb-2">
                                Поиск: {{ $query }}
                                <a href="{{ route('search', array_merge(request()->except('query'), ['page' => null])) }}" class="text-white ms-1">
                        ...
                        @if(request()->has('category') && request('category'))
                            ...
                            @if($categoryName)
                                <span class="badge bg-primary me-2 mb-2">
                                    ...
                                    <a href="{{ route('search', array_merge(request()->except('category'), ['page' => null])) }}" class="text-white ms-1">
                                        <i class="fas fa-times"></i>
                                    </a>
                                </span>
                            @endif
                        ...
                        @if(request()->has('cooking_time') && request('cooking_time'))
                            <span class="badge bg-primary me-2 mb-2">
                                ...
                                <a href="{{ route('search', array_merge(request()->except('cooking_time'), ['page' => null])) }}" class="text-white ms-1">
                                    <i class="fas fa-times"></i>
                                </a>
                            </span>
                        @endif
                        ...
                        @if(isset($sort) && $sort != 'relevance')
                            <span class="badge bg-primary me-2 mb-2">
                                ...
                                @elseif($sort == 'cooking_time_desc')
                                    По времени (убыв.)
                                @endif
                                <a href="{{ route('search', array_merge(request()->except('sort'), ['page' => null])) }}" class="text-white ms-1">
                                    <i class="fas fa-times"></i>
                                </a>
                            </span>
                        @endif
            ...
            <div class="search-results">
                                ...
                                <div class="card h-100 recipe-card border-0 shadow-sm">
                                    ...
                                    <div class="card-footer bg-white border-0">
                                        <a href="{{ route('recipes.show', $recipe->slug) }}" class="btn btn-primary w-100">
                                            <i class="fas fa-utensils me-1"></i> Смотреть рецепт
                                        </a>
                                    </div>
                                </div>
                            </div>
                ...
                @elseif(!empty($query))
                    <div class="alert alert-info">
                        <i class="fas fa-search me-2"></i> По запросу <strong>"{{ $query }}"</strong> ничего не найдено.
                    </div>
                    ...
                    <h3 class="h5 mt-4 mb-3">Популярные рецепты</h3>
                    <div class="row row-cols-1 row-cols-md-2 g-4">
                ...
                @else
                    <div class="alert alert-info mb-4">
                        <i class="fas fa-info-circle me-2"></i> Введите поисковый запрос, чтобы найти рецепты.
                    </div>
        ...
        <div class="col-lg-4">
                    ...
                    @if($categories->count() > 12)
                        <div class="text-center mt-3">
                            <a href="{{ route('categories.index') }}" class="btn btn-sm btn-outline-secondary">
                                <i class="fas fa-list me-1"></i> Все категории
                            </a>
                        </div>
                    @endif
                </div>
            ...
            <div class="card mb-4 border-0 shadow-sm">
                <div class="card-header bg-light">
                    <h3 class="h5 mb-0"><i class="fas fa-filter me-2"></i> Быстрые фильтры</h3>
                </div>
                <div class="card-body p-0">
                    <div class="list-group list-group-flush">
                        <a href="{{ route('search', ['cooking_time' => 15]) }}" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                            <div><i class="fas fa-bolt text-warning me-2"></i> Очень быстрые рецепты</div>
                        ...
                        <a href="{{ route('search', ['cooking_time' => 30]) }}" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                            <div><i class="fas fa-clock text-success me-2"></i> Быстрые рецепты</div>
                        ...
                        <a href="{{ route('search', ['sort' => 'popular']) }}" class="list-group-item list-group-item-action">
                            <i class="fas fa-fire text-danger me-2"></i> Популярные рецепты
                        </a>
                        <a href="{{ route('search', ['sort' => 'latest']) }}" class="list-group-item list-group-item-action">
                            <i class="fas fa-calendar-alt text-primary me-2"></i> Новые рецепты
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-light">
                    <h3 class="h5 mb-0"><i class="fas fa-carrot me-2"></i> Популярные ингредиенты</h3>
                </div>
                <div class="card-body">
                    <div class="row g-2">
